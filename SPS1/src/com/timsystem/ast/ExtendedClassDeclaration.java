...
public class ExtendedClassDeclaration implements Statement {
    ...
    public ExtendedClassDeclaration(ClassValue result, String name, List<String> argNames, Map<String, Expression> targets, String extension) {
        this.result = result;
        this.name = name;
    ...
    public void execute() {
        for (Map.Entry<String, Expression> entry : targets.entrySet()) {
            ...
            if (expr instanceof FunctionValue) {
                ...
                result.setField(entry.getKey(), new FunctionValue(new ClassMethod(fun.arguments, fun.body, result)));
                continue;
            }
        ...
        temporal = null;
        try {
            result.fields.putAll(((ClassValue) Variables.get(extension)).fields);
        ...
        Functions.set(name, (args) -> {
            ...
            for (Map.Entry<String, Expression> entry : targets.entrySet()) {
                ...
                if (expr instanceof FunctionValue) {
                    UserDefinedFunction fun = (UserDefinedFunction) ((FunctionValue) expr).getValue();
                    res.setField(entry.getKey(), new FunctionValue(new ClassMethod(fun.arguments, fun.body, res)));
                    continue;
                }
