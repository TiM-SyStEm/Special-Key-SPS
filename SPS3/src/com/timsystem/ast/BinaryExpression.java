...
public final class BinaryExpression implements Expression {
    ...
    private Value add(Value value1, Value value2) {
        ...
        else if (value1 instanceof ArrayValue) {
            return add((ArrayValue) value1, value2);
        }
        else return new StringValue((value1.toString() + value2.toString()).getBytes(StandardCharsets.UTF_8));
    }

    private ArrayValue add(ArrayValue value1, Value value2) {
        if (!(value2 instanceof ArrayValue)) throw new SPKException("BadArgException", "Expected array as second operand");
        Value[] arr1 = value1.array();
        Value[] arr2 = ((ArrayValue) value2).array();
        ArrayValue arr = new ArrayValue(arr1);
        for (Value val : arr2) {
            arr.append(val);
        }
        return arr;
