...
public class STL {
    ...
    private static void initMathClass() {
        ...
        math.put("asin", new FunctionValue(args -> {
            Arguments.check(1, args.length);
        ...
        math.put("cosh", new FunctionValue(args -> {
            Arguments.check(1, args.length);
            return new NumberValue(Math.cosh(args[0].asNumber()));
        }));
        math.put("acos", new FunctionValue(args -> {
            Arguments.check(1, args.length);
        ...
        math.put("tanh", new FunctionValue(args -> {
            Arguments.check(1, args.length);
            return new NumberValue(Math.tanh(args[0].asNumber()));
        }));
        math.put("atan", new FunctionValue(args -> {
            Arguments.check(1, args.length);
        ...
        math.put("toDegrees", new FunctionValue(args -> {
            Arguments.check(1, args.length);
            return new NumberValue(Math.toDegrees(args[0].asNumber()));
        }));
        math.put("toRadians", new FunctionValue(args -> {
            Arguments.check(1, args.length);
            return new NumberValue(Math.toRadians(args[0].asNumber()));
        }));
        math.put("isNegative", new FunctionValue(args -> {
            Arguments.check(1, args.length);
            if(Math.signum(args[0].asNumber())==-1)
                return NumberValue.ONE;
            return NumberValue.ZERO;
        }));
        math.put("isPositive", new FunctionValue(args -> {
            Arguments.check(1, args.length);
            if(Math.signum(args[0].asNumber())==1)
                return NumberValue.ONE;
            return NumberValue.ZERO;
        }));
        math.put("numType", new FunctionValue(args -> {
            Arguments.check(1, args.length);
        ...
        math.put("toHex", new FunctionValue(args -> {
            Arguments.check(2, args.length);
            String str = args[0].toString();
            String ch = args[1].toString();
            if (ch.equals("hex")){
                Variables.set("encodingHex", new StringValue((str).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else if (ch.equals("oct")){
                int dec = Integer.parseInt(str, 8);
                String k = Integer.toHexString(dec).toUpperCase();
                Variables.set("encodingHexO", new StringValue((k).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else if (ch.equals("dec")){
                int dec = Integer.parseInt(str, 10);
                String k = Integer.toHexString(dec).toUpperCase();
                Variables.set("encodingHexD", new StringValue((k).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else if (ch.equals("bin")){
                int dec = Integer.parseInt(str, 2);
                String k = Integer.toHexString(dec).toUpperCase();
                Variables.set("encodingHexB", new StringValue((k).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else {
                System.out.println("you specified an incorrect encoding");
            }
            return NumberValue.MINUS_ONE;
        }));
        math.put("toOct", new FunctionValue(args -> {
            Arguments.check(2, args.length);
            String str = args[0].toString();
            String ch = args[1].toString();
            if (ch.equals("oct")){
                Variables.set("encodingOct", new StringValue((str).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else if (ch.equals("hex")){
                int dec = Integer.parseInt(str, 16);
                String k = Integer.toOctalString(dec).toUpperCase();
                Variables.set("encodingOctH", new StringValue((k).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else if (ch.equals("dec")){
                int dec = Integer.parseInt(str, 10);
                String k = Integer.toOctalString(dec);
                Variables.set("encodingOctD", new StringValue((k).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else if (ch.equals("bin")){
                int dec = Integer.parseInt(str, 2);
                String k = Integer.toOctalString(dec);
                Variables.set("encodingOctB", new StringValue((k).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else {
                System.out.println("you specified an incorrect encoding");
        ...
        math.put("toBin", new FunctionValue(args -> {
            Arguments.check(2, args.length);
            String str = args[0].toString();
            String ch = args[1].toString();
            if (ch.equals("bin")){
                Variables.set("encodingBin", new StringValue((str).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else if (ch.equals("oct")){
                int dec = Integer.parseInt(str, 8);
                String k = Integer.toBinaryString(dec);
                Variables.set("encodingBinO", new StringValue((k).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else if (ch.equals("dec")){
                int dec = Integer.parseInt(str, 10);
                String k = Integer.toBinaryString(dec);
                Variables.set("encodingBinD", new StringValue((k).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else if (ch.equals("hex")){
                int dec = Integer.parseInt(str, 16);
                String k = Integer.toBinaryString(dec).toUpperCase();
                Variables.set("encodingBinH", new StringValue((k).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else {
                System.out.println("you specified an incorrect encoding");
        ...
        math.put("toDec", new FunctionValue(args -> {
            Arguments.check(2, args.length);
            String str = args[0].toString();
            String ch = args[1].toString();
            if (ch.equals("dec")){
                Variables.set("encodingDec", new StringValue((str).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else if (ch.equals("oct")){
                int dec = Integer.parseInt(str, 8);
                String k = Integer.toString(dec);
                Variables.set("encodingDecO", new StringValue((k).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else if (ch.equals("hex")){
                int dec = Integer.parseInt(str, 16);
                String k = Integer.toString(dec);
                Variables.set("encodingDecH", new StringValue((k).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else if (ch.equals("bin")){
                int dec = Integer.parseInt(str, 2);
                String k = Integer.toString(dec);
                Variables.set("encodingDecB", new StringValue((k).getBytes(StandardCharsets.UTF_8)));
                return NumberValue.MINUS_ONE;
            }
            else {
                System.out.println("you specified an incorrect encoding");
