...
import com.timsystem.ast.FunctionReferenceExpression;
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (spec != null) {
                    if (!specEquals(v, spec.getTypes().get(i)))
                        throw new SPKException("SpecificationError", "Expected " + spec + ", got " + specify(values));
                }
                Variables.define(getArgsName(i), v);
            }
            Variables.define("selfref", new FunctionValue(this));
            body.execute();
