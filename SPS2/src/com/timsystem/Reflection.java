...
public final class Reflection {
    ...
    private static class ClassValue extends MapValue {
        ...
        private void init(Class<?> clazz) {
            ...
            set("getComponentType", (Value) new FunctionValue(v -> classOrNull(clazz.getComponentType()) ));
            set("getDeclaringClass", (Value) new FunctionValue(v -> classOrNull(clazz.getDeclaringClass()) ));
            set("getEnclosingClass", (Value) new FunctionValue(v -> classOrNull(clazz.getEnclosingClass()) ));
            set("getSuperclass", (Value) new FunctionValue(v -> new ClassValue(clazz.getSuperclass()) ));

            set("getClasses", (Value) new FunctionValue(v -> array(clazz.getClasses()) ));
            set("getDeclaredClasses", (Value) new FunctionValue(v -> array(clazz.getDeclaredClasses()) ));
            set("getInterfaces", (Value) new FunctionValue(v -> array(clazz.getInterfaces()) ));

            set("asSubclass", (Value) new FunctionValue(this::asSubclass));
            set("isAssignableFrom", (Value) new FunctionValue(this::isAssignableFrom));
            set("new", (Value) new FunctionValue(this::newInstance));
            set("cast", (Value) new FunctionValue(this::cast));
        }
